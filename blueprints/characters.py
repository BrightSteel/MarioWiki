...
def characters():
  ...
  if tag:
    ...
    posts = db.get_posts_by_tag_and_category(tag, "characters", page=page)
    page_count = utils.get_total_pages(db.get_count_by_tag_and_category(tag, "characters")[0])

    all_posts = db.get_all_posts_by_category("characters")
  ...
  if title:
    database = db.get_post_by_title(title)
    try:
      content = database[1]
  ...
  trending_posts = db.get_trending_posts("characters")
  trending = []
  ...
  user_ids = db.get_top_contributors_by_category("characters")
  top_contributors = []
  for id in user_ids:
