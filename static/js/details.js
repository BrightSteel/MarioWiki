...
window.addEventListener("DOMContentLoaded", function() {
  ...
  let quillContainer = document.getElementById("editor-post")
  if (quillContainer) {
  ...
  let postContainer = document.getElementsByClassName("post_container");
  let id = postContainer[0].getAttribute("data-id");
  let formId = "form" + id;
  ...
  async function showEdit(){
    ...
    try {
      let post = await fetch('/post/postDetail/'+id);
  ...
  function submit_entry() {
    let hidden_input = document.getElementById('postData');
